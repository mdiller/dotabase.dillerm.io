...
--- criteria_subject
select c.pretty as label, group_concat(c.name, ' %'' OR r.criteria LIKE ''% ') as value, ('/vpk' || COALESCE(h.icon, '') || COALESCE(i.icon, '') ) as icon, 'width: 32px; height: 32px; margin: 4px' as icon_style, h.aliases as aliases
